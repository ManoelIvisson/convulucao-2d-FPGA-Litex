...
# --- Módulo de Interface com o RTL ---
class ConvolutionCore(Module, AutoCSR):
    def __init__(self, platform):
        # Registradores acessíveis via Software (C)
        self.pixel_in  = CSRStorage(8, description="Pixel de entrada (8-bit)")
        self.valid_in  = CSRStorage(1, description="Sinal de validade de entrada")
        
        self.pixel_out = CSRStatus(8, description="Pixel processado de saída")
        self.valid_out = CSRStatus(1, description="Sinal de validade de saída")
        ...
        self.specials += Instance("conv_top",
            # Conexões de Clock e Reset
            i_clk = i_clk,
            i_rstn = i_rstn,

            # Entrada de dados (do CSR para o Hardware)
            i_px_in  = self.pixel_in.storage,
            i_valid_in = self.valid_in.storage,

            # Saída de dados (do Hardware para o CSR)
            o_px_out  = self.pixel_out.status,
            o_valid_out = self.valid_out.status
        )
...
class BaseSoC(SoCCore):
    def __init__(self, board="i5", revision="7.2", toolchain="trellis", sys_clk_freq=60e6,
        ...
        **kwargs):
        ...
        # Adiciona nosso módulo de convolução ao SoC
        self.submodules.conv_core = ConvolutionCore(platform)
